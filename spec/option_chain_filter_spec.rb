...
RSpec.describe SchwabMCP::OptionChainFilter do
  ...
  describe '#find_spreads' do
    ...
    context 'with no valid options' do
      let(:invalid_options_map) do
        {
          "2025-01-17:2" => {
            "5790.0" => [{
              delta: -0.10,
              openInterest: 5, # Too low open interest
              strikePrice: 5790.0, # Too close to underlying
              mark: 5.50
            }]
          }
        }
  ...
  describe 'filtering behavior' do
    ...
    context 'date matching' do
      it 'finds spreads for matching expiration dates' do
        matching_options = {
          "2025-01-17:2" => {
            "5500.0" => [{ delta: -0.10, openInterest: 50, strikePrice: 5500.0, mark: 5.50 }],
            "5490.0" => [{ delta: -0.08, openInterest: 30, strikePrice: 5490.0, mark: 4.25 }]
          }
        }
      ...
      it 'works with symbol keys (as returned by JSON.parse with symbolize_names)' do
        symbol_key_options = {
          :"2025-01-17:2" => {
            "5500.0" => [{ delta: -0.10, openInterest: 50, strikePrice: 5500.0, mark: 5.50 }],
            "5490.0" => [{ delta: -0.08, openInterest: 30, strikePrice: 5490.0, mark: 4.25 }]
